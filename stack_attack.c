...
static const NotificationSequence snd_push = {
    TONE(1770, 70), TONE(1640, 80), REST(280), TONE(1770, 80), TONE(1850, 75), &message_sound_off,
    NULL};
static const NotificationSequence snd_blow = {TONE(1640, 130), TONE(1940, 135), &message_sound_off, NULL};
static const NotificationSequence snd_score = {
    TONE(1850, 10), TONE(4180, 55), REST(145), TONE(1770, 145), TONE(1980, 70), TONE(4690, 35),
    TONE(3920, 15), TONE(4690, 35), TONE(3920, 10), TONE(4690, 45), &message_sound_off, NULL};
static const NotificationSequence snd_over = {
    TONE(1810, 155), TONE(1680, 70), TONE(1980, 70), TONE(1850, 95), REST(50), TONE(1770, 310),
...
static void save_load(App* app) {
    for(int i = 0; i < OptCount; i++) app->opt[i] = true;
...
static void draw_callback(Canvas* canvas, void* ctx) {
    ...
    if(app->screen == ScreenTitle) {
        ...
        if(app->screen == ScreenPause) {
            ...
            canvas_set_font(canvas, FontSecondary);
            canvas_draw_str_aligned(
                canvas, 51, 36, AlignCenter, AlignCenter, app->new_record ? "New record!" : "Game over");
        }
    }
