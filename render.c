...
void fb_rect(uint8_t* fb, int x, int y, int w, int h, bool black) {
    for(int j = y; j < y + h; j++)
        for(int i = x; i < x + w; i++) fb_pixel(fb, i, j, black);
}
...
static const Sprite* worker_sprite(const Game* g) {
    ...
    bool left = w->face < 0;
    if(w->vstate != WGround)
        return left ? &spr_worker_jump_l : &spr_worker_jump_r;
    if(w->move_left) {
...
static void draw_cell(uint8_t* fb, const Game* g, int r, int c) {
    ...
    static const Sprite* const stages[G_CLEAR_CELL_TICKS] = {
        &spr_clear1, &spr_clear1, &spr_boom2, &spr_boom2, &spr_boom2, &spr_boom3,
        &spr_boom3, &spr_boom3, &spr_boom3, &spr_boom4, &spr_boom4, &spr_boom5};
...
// Side panel, one line every 7 px: label, value right-aligned below it.
static void draw_panel(uint8_t* fb, const Game* g, uint32_t hiscore) {
    char buf[12];
    const int x = G_FIELD_W + 2;
    fb_text(fb, x, 3, "SCORE");
...
void render_game(uint8_t* fb, const Game* g, uint32_t hiscore) {
    ...
    for(int r = 0; r < G_ROWS; r++)
        for(int c = 0; c < G_COLS; c++) draw_cell(fb, g, r, c);
    ...
    if(g->debris_timer) blit(fb, &spr_debris, g->debris_x, g->debris_y - (G_DEBRIS_TICKS - g->debris_timer));
